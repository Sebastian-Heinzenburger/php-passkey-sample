...
try {
    ...
    if ($data['step'] === 'options') {
        ...
        $options = json_decode(json_encode($publicKeyCredentialCreationOptions), true);
        ...
        // Insert user
        $stmt = $db->prepare('INSERT INTO users (username, user_id) VALUES (?, ?)');
        $stmt->execute([$_SESSION['username'], $_SESSION['userId']]);
        $userId = $db->lastInsertId();

        // Insert credential
        $stmt = $db->prepare('INSERT INTO credentials (user_id, credential_id, credential_raw_id, public_key) VALUES (?, ?, ?, ?)');
        $stmt->execute([
            $userId,
            $data['credential']['id'],
